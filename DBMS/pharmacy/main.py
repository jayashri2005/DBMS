...
def order_delete(oid1):
    c.execute(''' DELETE FROM Orders WHERE O_id = ?''', (oid1,))
...
def getauthenicate(username, password):
    ...
    if cust_password:  # Check if the list is not empty
...
def customer(username, password):
    if getauthenicate(username, password):
        ...
        if drug_result:  # Check if drug_result is not empty
            ...
            st.error("No drug information available.")
